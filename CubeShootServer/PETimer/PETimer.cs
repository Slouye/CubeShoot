/****************************************************
    ...
    答：统一换算成最小的毫秒运算
    4.如何支持多线程定时任务
    答：使用System.Timers类的Elapsed每隔一定时间来检测Update。通过临时列表来进行缓存，错开操作时间，尽可能的使用锁，提示操作效率。
    5.如果实现基础定时任务
    答：通过Update（）来检测任务条件。
*****************************************************/
...
public class PETimer
{
    ...
    //增加临时列表的原因是在保证数据安全的情况下不浪费性能。（免得一直。。加锁）
        ...
        for (int index = 0; index < taskTimeLst.Count; index++)
        {
            ...
            else
            {
                ...
                try
                {
                    if (taskHandle != null)
                    {
                        //处理器添加这个回调，（定时条件满足，taskHandle不为空）
    ...
    {   //不使用DateTime.Now 因为= = 卡断点的时候也一直获取的是当前时间。
